/*
 * LutNetworkFixedPointRegression.h
 *
 *  Created on: Mar 16, 2021
 *      Author: kbunkow
 */

#ifndef INTERFACE_LUTNETWORKFIXEDPOINTREGRESSION_H_
#define INTERFACE_LUTNETWORKFIXEDPOINTREGRESSION_H_
...
class LutNetworkFixedPointRegression {
public:
    LutNetworkFixedPointRegression(): outputNode(nullptr) {
    ...
    LutNetworkFixedPointRegression(NetworkOutNode* outputNode): outputNode(outputNode) {
        std::cout<<"LutNetworkFixedPoint"<<std::endl;
    };
    virtual ~LutNetworkFixedPointRegression() {};
    ...
    typedef LutNeuronLayerFixedPoint<layer3_input_I, layer3_input_F, layer2_neurons, layer3_lut_I, layer3_lut_F, layer3_neurons, output_I> LutLayer3;
    ...
    auto& runWithInterpolation() {
        ...
        std::array<ap_ufixed<layer2_input_I + layer2_input_F, layer2_input_I, AP_RND, AP_SAT> , layer1_neurons> layer1OutWithBias;
    ...
    void run(Event<InputType>* event) {
        ...
        for(unsigned int iOut = 0; iOut < outputFixedPoint.size(); iOut++) {
            event->nnResult[iOut] = outputFixedPoint[iOut];
        }
    }
...
private:
    std::array<ap_ufixed<LutLayer1::input_W, input_I, AP_RND, AP_SAT> , inputSize> inputArray;
...
#endif /* INTERFACE_LUTNETWORKFIXEDPOINTREGRESSION_H_ */
