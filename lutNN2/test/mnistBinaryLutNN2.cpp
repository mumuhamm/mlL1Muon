...
int main(void) {
	...
	unsigned int branchesPerOutput = 70;
...
/*	//layer 0
	...
	layerConfig.bitsPerNodeInput = 2;
	layerConfig.nodeInputCnt = 3;
    ...
    for(unsigned int i = 0; i < iterations; i++) {
...

        {
        	...
        	for(unsigned int iEv = 0; iEv < trainingEventsBatch.size(); iEv++) {
        		...
        		//network.runTraining(trainingEventsBatch[iEv], costFunction);  //TODO!!!!!!!!!!!!!!!!!!!!
        		//cout<<"line "<<dec<<__LINE__<<endl;
        		//network.runTrainingAndUpdate(&eventWithNoise, costFunction, learnigParamsVec); //TODO!!!!!!!!!!!!!!!!!!!!
        		//cout<<"line "<<dec<<__LINE__<<endl;
        	}
        ...
        if( (i)%printEveryIteration == 0) {
        	...
        	layersConf[layersConf.size()-2]->ditherRate = 45; //this is in percent!!!!!
            //cout<<"line "<<__LINE__ << timer1.format() << '\n';
