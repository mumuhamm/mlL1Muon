...
int main(void) {
    ...
    //string rootFile1 = "/afs/cern.ch/work/k/kbunkow/public/CMSSW/cmssw_12_x_x_official/CMSSW_12_3_0_pre4/src/L1Trigger/L1TMuonOverlapPhase1/test/expert/omtf/omtfHitsDump.root";

    string rootFileTest1 = "/afs/cern.ch/work/k/kbunkow/public/CMSSW/cmssw_12_x_x_official/CMSSW_12_3_0_pre4/src/L1Trigger/L1TMuonOverlapPhase1/test/expert/omtf/omtfHitsDump.root";
   ...
   // eventsGeneratorTrain.readEvents(dataPath + "OMTFHits_pats0x00012_newerSample_files_1_80.root");
    ...
    //eventsGeneratorTrain.readEvents(dataPath + "OMTFHits_pats0x00012_oldSample_files_1_15_PtUpTo13GeV.root");
...
/*    dataPath = "/afs/cern.ch/work/k/kbunkow/public/CMSSW/cmssw_12_x_x_official/CMSSW_12_3_0_pre4/src/L1Trigger/L1TMuonOverlapPhase1/test/crab/";
    ...
    for(int iFile = 1; iFile <= 50; iFile++)  //TODO!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        eventsGeneratorTrain.readEvents(datdDir + "omtfHitsDump_" + to_string(iFile) + ".root");
...
/*
	...
	unsigned int iterations = 15000; //15000;
    ...
    lutLayerConfig.initSlopeMax = (lutLayerConfig.maxLutVal - lutLayerConfig.minLutVal) / (lutSize/lutLayerConfig.lutRangesCnt) / 15.;
    ...
    lastLayer.back()->setName(lastLayer.back()->getName() + "layer_"  + std::to_string(5) + "_node_" + std::to_string(9) + "_charge");

    if(outputCnt > 2) {
        lastLayer.emplace_back(networkBuilder.getNode(&sumLayerConfig, 1));
        lastLayer.back()->setName(lastLayer.back()->getName() + "layer_"  + std::to_string(5) + "_node_" + std::to_string(9) + "_isMuon");
    }
    ...
    size_t validationEventCnt = 10000;
    if(validationEventCnt > eventsGeneratorTest.getEvents().size())
        validationEventCnt = eventsGeneratorTest.getEvents().size();

    vector<EventFloat*> validationEvents(validationEventCnt, nullptr);
    ...
    omtfAnalyzer.muonAlgorithms.emplace_back(new LutNNRegressionAlgorithm(17, 12, "lutNN_Regression_HighQ", kMagenta));
    omtfAnalyzer.muonAlgorithms.emplace_back(new LutNNRegressionAlgorithm(17, 1, "lutNN_Regression", kMagenta));
    omtfAnalyzer.muonAlgorithms.emplace_back(new LutNNRegressionAlgorithm(18, 12, "lutNN_Regression_HighQ", kMagenta));
    omtfAnalyzer.muonAlgorithms.emplace_back(new LutNNRegressionAlgorithm(18, 1, "lutNN_Regression", kMagenta));
    omtfAnalyzer.muonAlgorithms.emplace_back(new LutNNRegressionAlgorithm(19, 12, "lutNN_Regression_HighQ", kMagenta));
    omtfAnalyzer.muonAlgorithms.emplace_back(new LutNNRegressionAlgorithm(19, 1, "lutNN_Regression", kMagenta));
