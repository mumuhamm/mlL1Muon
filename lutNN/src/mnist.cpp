...
int main(void) {
    ...
    ConfigParameters::LutType lutTupe = ConfigParameters::interpolated;

    layersDef.emplace_back(ConfigParametersPtr(new ConfigParameters(neuronCntL1,               lutSize, lutSize, inputCnt, ConfigParameters::fullyConnected, lutTupe) ) );
    layersDef.emplace_back(ConfigParametersPtr(new ConfigParameters(outputCnt,                 lutSize, 0      , neuronCntL1, ConfigParameters::fullyConnected, lutTupe) ) );
...
/*    for(unsigned int iLayer = 0; iLayer < lutNetwork.getNeuronLayers().size(); iLayer++) {
    ...
    for(int i = 0; i < iterations; i++) {
        ...
        if(i == 400) {
            speed = 12;
            //wrongClassEventWeight = 3;
            learnigParamsVec[0].learnigRate = speed * 1.;//0.5;//0.5;//2.;
            learnigParamsVec[0].beta = 0.0;
            learnigParamsVec[0].lambda = 0.000001;
            learnigParamsVec[0].smoothWeight = 0;// * learnigParams.learnigRate;
            learnigParamsVec[0].maxLutVal = 3;

            learnigParamsVec[1].learnigRate = speed * 1.;
            learnigParamsVec[1].beta = 0.0;
            learnigParamsVec[1].lambda = 0.000001;
            learnigParamsVec[1].smoothWeight = 10;//0.1;// * learnigParams.learnigRate;
            learnigParamsVec[1].maxLutVal = 5;
        ...
        if(i == 4000) {
            ...
            learnigParamsVec[0].beta = 0.92;
            learnigParamsVec[1].beta = 0.92;
        }
        if(i == 6000) {
      ...
      /*  if(i == 8000) {
        ...
        for(auto& event : trainingEvents) {
            ...
            if(lutTupe == ConfigParameters::interpolated)
                lutNetwork.runTrainingInter(event->expextedResult, costFunction);
            else if(lutTupe == ConfigParameters::discrete)
