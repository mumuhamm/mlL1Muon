 ...
 /*   for(unsigned int iLayer = 0; iLayer < layersDef.size(); iLayer++ ) {
    ...
    for(int i = 0; i < iterations; i++) {
        ...
        if(i == 0) { //not sure if needed
            learnigParamsVec[0].learnigRate = speed * 10;//0.5;//0.5;//2.;
            learnigParamsVec[0].beta = 0.95;
            learnigParamsVec[0].lambda = 0.0005;
        ...
        if(i == 1000 ) { //not sure if needed
            double speed = 0.2;
            learnigParamsVec[0].learnigRate = speed * 20;//0.5;//0.5;//2.;
            learnigParamsVec[0].beta = 0.95;
            learnigParamsVec[0].lambda = 0.0005;
            learnigParamsVec[0].regularizationRate = 0;// * learnigParams.learnigRate;
